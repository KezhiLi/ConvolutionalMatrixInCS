function [U,Out] = RecPF_Circ_Eq(m,n,aTV,aL1,picks,Mask,B,TVtype,opts,PsiT,Psi,URange,uOrg)
% [U,Out] = RecPF_Circ_Eq(m,n,aTV,aL1,picks,B,TVtype,opts,PsiT,Psi,URange,uOrg)
%
% RecPC solves the TVL1-L2 model:
%
%   min aTV*TV(u) + aL1*|PsiT*U|_1, subject to P*C*U = B
%
...
%% initial vars U, etc.
% If changing U to anything nonzeor, you MUST change the
...
if exist('uOrg','var'); snr1(U,uOrg); end % initial SNR computation
...
if opts.normalize
    ...
    fctr = 1/URange;
    B = fctr*B;
    if exist('uOrg','var'); uOrg = fctr*uOrg; snr1(U,uOrg); end
...
if(0)
    fprintf('condition number Denom1 = %e\n',max(Denom1(:))/min(Denom1(:)));
    fprintf('condition number Denom2 = %e   Max Denom2 = %e\n',max(Denom2(:))/min(Denom2(:)),max(Denom2(:)));
    fprintf('condition number Denom = %e\n',max(Denom(:))/min(Denom(:)));
    return;
%     disp('Denom truncation.');
%     tmp = 1e-6;
%     Denom(Denom<tmp)=tmp;
%     clear tmp;
end
...
for ii = 1:maxItr
    ...
    %    if exist('uOrg','var'); fprintf(' snr=%4.1f',snr1(U)); end
